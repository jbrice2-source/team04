...
class TakeImages:
    ...
    def random_move(self, *args):
        ...
        if np.linalg.norm([self.pos.x,self.pos.y]) > 0.9-self.add_dist:
            ...
            self.add_dist = 0.1
            self.cur_target = None
        elif np.linalg.norm([self.pos.x-self.pos2.x,self.pos.y-self.pos2.y]) < 0.35+self.add_dist:
            self.velocity.twist.linear.x = -0.18
            self.velocity.twist.angular.z = 0.0
            self.add_dist = 0.1
            self.cur_target = None
    ...
    def camera_move(self, *args):
        ...
        target = np.arctan2(self.pos.y-self.pos2.y,self.pos.x-self.pos2.x)%(2*np.pi)#self.targets[self.cur_target]*np.pi/180
          
        dists = [(self.pos2.theta%(2*np.pi)-target%(2*np.pi))%(2*np.pi),(target%(2*np.pi)-self.pos2.theta%(2*np.pi))%(2*np.pi)]
        # print(self.pos.theta, np.round(dists,2), np.argmin(dists), target, self.velocity2.twist.angular.z)
        ...
        if min(dists) < 0.2:
            ...
            self.velocity2.twist.angular.z = 0.4
            # print("turning left")
        else:
            self.velocity2.twist.angular.z = -0.4
            # print("turning right")
    ...
    def take_picture(self):
        ...
        if type(images[0]) != type(None) and type(images[1]) != type(None):
            for i in range(len(images)):
                ...
                # id, camera index, pos1x, pos1y, pos1theta, pos2x, pos2y, pos2theta
                params = [img_id,i,self.pos.x,self.pos.y,self.pos.theta,self.pos2.x,self.pos2.y,self.pos2.theta]
                cv2.imwrite(f"dataset/{img_id}.png", images[i])
