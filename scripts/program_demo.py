...
class Explore:
    def __init__(self):
        ...
        plt.subplot(224)
        self.display4 = plt.imshow(self.map, vmin=0,vmax=255)
        plt.show()
    ...
    def other_pose(self,pose):
        ...
        if self.pos is not None and pose is not None:
            ...
            # checks if the miro has found the lost one
            if distance < 0.7 and not self.miro_found and not self.starting_scan:
                ...
                for i,n in enumerate(self.map):
                    for j,m in enumerate(n):
                        if (m == m[0]).all() and (m[0] < 150):
                            self.final_map[i,j] = True
                display_map = self.final_map.copy().astype(int)
                self.display4.set_data((1-display_map)*255)
                plt.draw()
    ...
    def follow_miro(self, *args):
        ...
        # calculates the target angle for the position
        target_pos = np.array([self.other_pos.x,self.other_pos.y])
        self.velocity.twist.linear.x = 0.2
        target = np.arctan2(target_pos[1]-self.pos.y,target_pos[0]-self.pos.x)
    ...
    def make_sound(self):
        ...
        #East
            ...
            self.pub_stream.publish(load_wav(FREQUENCY_PATH + "2400_hz.wav"))
